...
import { QueueEventType, QueueItemStatus, VoicevoxQueueManager } from './queue'
 ...
 */
function withErrorHandling<R>(method: () => Promise<R>, errorMessage: string): Promise<R> {
...
export class VoicevoxPlayer {
   ...
   * @param speaker 話者ID
   */
  public async enqueue(text: string, speaker = 1): Promise<void> {
    return withErrorHandling(async () => {
   ...
   * @param speaker 話者ID
   */
  public async enqueueWithQuery(query: AudioQuery, speaker = 1): Promise<void> {
    return withErrorHandling(async () => {
   ...
   * @param speaker 話者ID
   */
  public async synthesizeToFile(query: AudioQuery, output?: string, speaker = 1): Promise<string> {
    return withErrorHandling(async () => {
      // クエリから音声を合成
  ...
  public isPlaying(): boolean {
    return this.queueManager.getQueue().some((item) => item.status === QueueItemStatus.PLAYING)
  }
