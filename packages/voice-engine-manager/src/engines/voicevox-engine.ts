...
export class VoicevoxEngine extends BaseEngine {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      if (!isOnline && this.config.boot_command === "deny") {
        throw new Error(`VOICEVOX engine "${this.config.name}" is not running and boot_command is "deny"`);
      }
  ...
  getDefaultLaunchCommand(): string {
    return process.platform === "win32" 
      ? "VOICEVOX.exe --no-sandbox" 
      : "./VOICEVOX";
  }

  async findExecutableAndGenerateCommand(): Promise<string> {
    try {
      const command = await this.executableFinder.generateLaunchCommand(KNOWN_ENGINE_TYPES.VOICEVOX);
      console.log(`VOICEVOX engine "${this.config.name}": Using command: ${command}`);
      return command;
    } catch (error) {
      console.warn(`VOICEVOX engine "${this.config.name}": Failed to find executable, falling back to default command. Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
  ...
  getDefaultUrl(): string {
    return DEFAULT_ENGINE_URLS[KNOWN_ENGINE_TYPES.VOICEVOX];
  }

  getDefaultSpeaker(): number | string {
    return DEFAULT_SPEAKERS[KNOWN_ENGINE_TYPES.VOICEVOX];
  }
}